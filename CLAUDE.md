...
- **Takes averaged; time views follow the selection BUILT (2026-09-06, branch `e-feedback`, user
  ...
  **take 0 carries the mean in place** (`refreshSlotMean(i)` on every landing / removal /
  ...
  The add row reads *+ Add another take of this guitar*. **Each spectrogram pane and the envelope
  plot carry their own Take picker beside the plot** (`.takebar.takesel` as the plot wrap's first
  child — A's above pane A, B's above pane B, both above the envelope; hidden for a single take;
  `syncTakeSels()` reads/writes `selTake` through `selectTake` — one state; `syncMagCtls` carries
  the opened pane's bar into the expanded view and back). **Safari after a screen lock:** a
  context that saw the page hidden (or WebKit's `interrupted`) is **spent** — `startPlayback`
  closes it and builds a new one (`playCtxSpent`); an idle `recLive.ctx` is dropped on hidden
  too, a live capture never. **"Before you compare" (user, same day):** the Take group and the
  comparability bar are one block at the **top** of the tone rows (`preCompareHtml`): a line per
  guitar — Level and floor · Pitch check · Material · Recording path, lights + phrases, every one
  a readout tap — and a last line that always answers (*Fair to compare* / *Not directly
  comparable — register (7.0 semitones apart) …* / load the other guitar). Dynamic range and
  Between notes are **readouts now, not rows** (inside Level and floor / Recording path);
  `TONE_GROUPS` is two groups, the take defs keep `g:"take"` (`TONE_PRE_GROUP`); `#toneCompat`
  is gone. **Sustain and Dead spots are one row** (`f0-decay`, the flags in its detail; the
...
- **Visibility batch BUILT (2026-09-06, branch `e-feedback`).** `APP_URL`/`APP_REPO`/`APP_HOST`
  printed on every PNG footer, CSV header, the JSON snapshot and the EQ settings text; `<head>`
  carries description, canonical, Open Graph + Twitter card with `docs/img/og.png`; **Share image**
  (1200×630: the spectrum scene + At a glance + the address, `exportShareImage`) and **Copy link**
  under At a glance; the demo declares itself solidbody; `?scrollto=<id>` and a deferred
  `?feedback` (`runLandingHooks()`); README rewritten around `docs/img/*.png` with the live link
  first and the analytics claim corrected; GitHub topics/description/homepage set via `gh`. Owner
  to do: social preview upload, push, release. `tests/e.test.js` **305**.
- **Demo pair rebuilt (2026-09-06, branch `e-feedback`; user request: two synths close to a
  Strat and a Les Paul "without naming names" that show the full feature set).** `loadDemo()`
  lands **two takes each** of *Single-coil 25.5″* (44.1 kHz) and *Humbucker 24.75″* (48 kHz) —
  named by construction, never by maker (an inverted contract keeps it so) — playing one phrase:
  a riff up the A string ending on D♯4/E4, a rest, the six open strings one at a time; 14 notes,
  every note muted before the next pluck. `demoString` is extended Karplus–Strong with a **shaped
  pluck** (not noise — noise leaves random harmonics that fail the comb check), a pick-position
  comb (0.14, the player) and a pickup-position comb (0.065 bridge single-coil vs 0.24 neck
  humbucker), T20-derived loop loss, HF loss, a per-note-scaled allpass for B, a quiet slow second
  polarisation for the two-stage decay, a click, a resonant pickup low-pass, −78 dBFS hiss; the
  single-coil guitar has a **dead spot at D♯4**. Rendered, the page says *Fair to compare* (4/4
  pairs), *Strings and scale differ* (B 1.03 vs 1.65), and gives verdicts on pickup voice,
  brightness, even/odd, richness, overtone ring, sustain and the dead spot, with bloom measured
  on both. The block is sentinel-fenced and **byte-identical in `tests/make_samples.js`**
  (asserted); `samples/demo-{singlecoil-44k,humbucker-48k}_take{1,2}.wav` replace the old pair;
  README/OG screenshots re-captured. Traps recorded in SPEC.md (pluck at 0.20 kills harmonic 5;
...
- The gate: `./tests/verify.sh` — dsp **236**, r3 42, r4 60, m27 51, r5 **332**, **e 187**, headless **73**
  (eight steps since E1; `--node` skips the headless step). On `e-feedback`: dsp **237**, e **305**. **All node suites green since the
...
- `samples/demo-singlecoil-44k_take{1,2}.wav`, `samples/demo-humbucker-48k_take{1,2}.wav` — the demo pair as files (two construction-modelled solidbodies, two takes each, the same phrase; regenerated by `tests/make_samples.js`), drag-drop test material.
...
- Open `index.html` in a browser. `?demo` auto-loads the built-in demo pair
  ...
  **drawn**, after the x-stride thinner — not the number of clusters found), all absent
  ...
  `?scrollto=<card id>` (scroll a card into view once both guitars have landed — a fragment
  cannot, the cards are hidden at load), `?feedback` (open the Feedback dialog, after the data lands
  when `?demo`/`?load` is present),
...
- **The tone panel reports the guitar, and says how sure it is — by shape, not by sentence
  ...
  beside the dots. Two row groups by the player's question (How it sounds / How it rings) under a *Before you
  compare* block that holds the take facts and the comparability verdict; only state-1 `glance`
  rows reach At a glance; "not distinguishable" is said
...
- **The address rides on everything that leaves the page.** `APP_URL`/`APP_REPO`/`APP_HOST` are the
  one place the address is written; PNG footers, CSV headers, the JSON snapshot, the EQ text and the
  share image all print from them. A new export prints them too.
